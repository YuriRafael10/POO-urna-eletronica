...
namespace urna
{
    ...
    {
        public static void AutoCadastrar(List<Partido> partidos, List<DepEstadual> deputadosEstaduais, List<DepFederal> deputadosFederais, List<Vereador> vereadores)
        {
            /*Auto cadastra politicos do legislativo*/
            int m = 10, num = 0;
            for (int i = 0; i < 5; i++)
            {
                string sigla = "PP" + i;
                for (int j = 0; j < 5; j++)
                {
                    string nome = "Vereador" + (num);
                    int idade = i + 30 + num;
                    Partido partido = AutoPartidoPolitico(partidos, sigla);
                    int n = partido.getLegenda();
                    string a = n + m.ToString();
                    int legenda = int.Parse(a);
                    vereadores.Add(new Vereador(nome, idade, partido, legenda));
                    partido.setVereadores(new Vereador(nome, idade, partido, legenda));
                    m++;
                    num++;
                }
            }
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                for (int j = 0; j < 5; j++)
                {
                    string nome = "DepEstadual" + (num);
                    int idade = i + 30 + num;
                    Partido partido = AutoPartidoPolitico(partidos, sigla);
                    int n = partido.getLegenda();
                    string a = n + m.ToString();
                    int legenda = int.Parse(a);
                    deputadosEstaduais.Add(new DepEstadual(nome, idade, partido, legenda));
                    partido.setDepEstaduais(new DepEstadual(nome, idade, partido, legenda));
                    m++;
                    num++;
                }
            }
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                for (int j = 0; j < 5; j++)
                {
                    string nome = "DepFederal" + (num);
                    int idade = i + 30 + num;
                    Partido partido = AutoPartidoPolitico(partidos, sigla);
                    int n = partido.getLegenda();
                    string a = n + m.ToString();
                    int legenda = int.Parse(a);
                    deputadosFederais.Add(new DepFederal(nome, idade, partido, legenda));
                    partido.setDepFederais(new DepFederal(nome, idade, partido, legenda));
                    m++;
                    num++;
                }
            }
