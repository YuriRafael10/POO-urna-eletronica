...
namespace urna
{
    ...
    {
        public static void AutoCadastrar(List<Partido> partidos, List<Presidente> presidentes, List<Gorvenador> gorvenadores, List<Prefeito> prefeitos)
        {
            for (int i = 0; i < 5; i++)
            {
                string nome = "Presidente" + i, sigla = "PP" + i;
                int idade = (i * 2) + 50;
                Partido partido = AutoPartidoPolitico(partidos, sigla);
                presidentes.Add(new Presidente(nome, idade, partido));
                partido.setPresidente(new Presidente(nome, idade, partido));
            }
            for (int i = 0; i < 5; i++)
            {
                string nome = "Gorvenador" + i, sigla = "PP" + i;
                int idade = (i * 2) + 50;
                Partido partido = AutoPartidoPolitico(partidos, sigla);
                gorvenadores.Add(new Gorvenador(nome, idade, partido));
                partido.setGorvenador(new Gorvenador(nome, idade, partido));
            }
            for (int i = 0; i < 5; i++)
            {
                string nome = "Prefeito" + i, sigla = "PP" + i;
                int idade = (i * 2) + 50;
                Partido partido = AutoPartidoPolitico(partidos, sigla);
                prefeitos.Add(new Prefeito(nome, idade, partido));
                partido.setPrefeito(new Prefeito(nome, idade, partido));
            }
        }
