...
namespace urna
{
    ...
    {
        public static void AutoCadastrarPartido(List<Partido> partidos)
        {
            for (int i = 0; i < 5; i++)
            {
                string nomepartido = "PARTIDO" + i, siglapartido = "PP" + i;
                int num = 10 + i;
                partidos.Add(new Partido(nomepartido, siglapartido, num));
            }
            Console.WriteLine("PARTIDOS CADASTRADOS");
